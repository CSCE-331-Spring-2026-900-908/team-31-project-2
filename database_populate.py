            ...
            for day in range(365):
                ...
                for order in range(num_orders):
                    ...
                    for i in range(numItems):
                        ...
                        modifier_writer.write(",".join([str(modifier_id), str(detail_id), str(ice), str(modifier_prices[ice]), modifiers[ice]]))
                        modifier_writer.write("\n")
                        modifier_id += 1
                        modifier_writer.write(",".join([str(modifier_id), str(detail_id), str(sugar), str(modifier_prices[sugar]), modifiers[sugar]]))
                        modifier_writer.write("\n")
                        modifier_id += 1
                        modifier_writer.write(",".join([str(modifier_id), str(detail_id), str(size), str(modifier_prices[size]), modifiers[size]]))
                        modifier_writer.write("\n")
                        modifier_id += 1
                        modifier_writer.write(",".join([str(modifier_id), str(detail_id), str(milk), str(modifier_prices[milk]), modifiers[milk]]))
                        modifier_writer.write("\n")
                        modifier_id += 1
                        for j in range(numToppings):
                            ...
                            modifier_writer.write(",".join([str(modifier_id), str(detail_id), str(topping_id), str(modifier_prices[topping_id]), modifiers[topping_id]]))
                            modifier_writer.write("\n")
                            modifier_id += 1
                        order_price += item_price
                        detail_writer.write(",".join([str(detail_id), str(order_id), str(product_id), str(item_price), snapshot]))
                        detail_writer.write("\n")
                        detail_id += 1
                    # total_tax = sum(menu_prices[item] for item in items)
                    order_writer.write(",".join([str(order_id), str(employee_id), str(date_time), str(order_price), f"{order_price*1.0825:.2f}"]))
                    order_writer.write("\n")
                    order_id += 1